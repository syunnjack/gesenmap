...
店舗を **アーケードゲーム / プライズ / カプセルトイ / プリクラ / その他** に分ける。
`/category`・`/category/{種類}`・`/area/{都道府県}/{種類}` で引ける。

**分類は公式が出しているものからしか決めない。**

| チェーン | 何を根拠にするか |
|---|---|
| GiGO | 店舗ページの「ゲームの種類」 |
| バンダイナムコ | 店舗ページの「設置ゲーム機」タブ（区分）と「入荷プライズ」タブの有無 |
| タイトー | 一覧APIの `StoreGroupID`（0=ゲームセンター / 2=brooming＝プリクラ専門 / 11=ガチャステ＝カプセルトイ専門 / 1=ボウリング）。**7＝マリオンクレープはゲームを置いていないので掲載しない** |
| イオンファンタジー | ブランド名が業態を表すものだけ（TOYS SPOT PALO・カプセル横丁＝カプセルトイ、PRIZE SPOT PALO・クレーン横丁＝プライズ） |

**モーリーファンタジーと PALO は、置いている機種が公式サイトに出ていないので分類しない。**
分類できなかった店舗は `categories` を空にして、画面に「未確認」と出す。推測で埋めないこと。
